  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0F0A1E,#1A0F3C,#0D1A2E)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ color:"rgba(255,255,255,0.4)", marginTop:16, fontFamily:"'Nunito',sans-serif", fontSize:14 }}>{text}</p>
      </div>
    </div>
  );
}
      ...
      <div className="glass" style={{ borderRadius:28, padding:"40px 32px", width:"100%", maxWidth:420, animation:"slideUp 0.6s ease" }}>
        {error ? (
          ...
          <>
            ...
            <p style={{ color:"rgba(255,255,255,0.4)", fontSize:14, textAlign:"center", marginBottom:6 }}>
              Join <strong style={{ color:"#A78BFA" }}>{board?.board_name}</strong>
            </p>
            <p style={{ color:"rgba(255,255,255,0.3)", fontSize:13, textAlign:"center", marginBottom:28 }}>
              Members: {(board?.members||[]).map(m => `${m.avatar} ${m.name}`).join(", ")}
            </p>
...
function SetupScreen({ onComplete }) {
  ...
  const [members, setMembers] = useState([{ name:"", avatar:"🦁" }]);
  ...
  const handleModeSelect = (key) => {
    setMode(key);
    const count = key === MODES.DUO ? 2 : 1;
    setMembers(Array.from({ length: count }, (_, i) => ({ name:"", avatar: AVATARS[i] })));
  };

  const updateMember = (i, field, val) => setMembers(p => p.map((m, idx) => idx===i ? { ...m, [field]:val } : m));
  const addMember = () => { if (members.length < 5) setMembers(p => [...p, { name:"", avatar: AVATARS[p.length] }]); };
  const canProceed = mode && boardName.trim() && members.every(m => m.name.trim());
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const boardData = {
        ...
        mode,
        members: members.map((m, i) => ({ ...m, id: `member_${i}`, name: m.name.trim() })),
        checkins: {},
        goals: [],
        created_at: getToday(),
      };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0F0A1E 0%,#1A0F3C 50%,#0D1A2E 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'Nunito',sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      <div className="glass" style={{ borderRadius:28, padding:"40px 32px", width:"100%", maxWidth:440, animation:"slideUp 0.6s ease" }}>
        ...
        {error && (
          <div style={{ background:"rgba(252,165,165,0.15)", border:"1px solid rgba(252,165,165,0.3)", borderRadius:12, padding:"12px 16px", marginBottom:16 }}>
        ...
        {mode && (
          <div>
            <p style={{ fontSize:11, color:"rgba(255,255,255,0.4)", fontWeight:700, textTransform:"uppercase", letterSpacing:1, marginBottom:10 }}>Members</p>
            {members.map((m, i) => (
              <div key={i} style={{ display:"flex", gap:10, marginBottom:10, alignItems:"center" }}>
                <div style={{ display:"flex", gap:6, background:"rgba(255,255,255,0.06)", borderRadius:12, padding:"6px 8px", flexShrink:0 }}>
                  {AVATARS.slice(0, 6).map(av => (
                    <span key={av} onClick={() => updateMember(i, "avatar", av)}
                      style={{ fontSize:18, cursor:"pointer", opacity: m.avatar===av?1:0.3, transition:"opacity 0.15s" }}>{av}</span>
                  ))}
                </div>
                <input className="s-input" placeholder={`Member ${i+1} name`} value={m.name} onChange={e => updateMember(i, "name", e.target.value)} />
              </div>
            ))}
            {mode === MODES.GROUP && members.length < 5 && (
              <button onClick={addMember} style={{ background:"rgba(255,255,255,0.06)", border:"1px dashed rgba(255,255,255,0.2)", borderRadius:12, color:"rgba(255,255,255,0.5)", padding:"10px", width:"100%", cursor:"pointer", fontSize:13, fontFamily:"'Nunito',sans-serif", marginBottom:4 }}>
                + Add Member
              </button>
            )}
          </div>
        ...
        <button onClick={handleComplete} disabled={!canProceed || creating}
          style={{ width:"100%", marginTop:16, background: canProceed?"linear-gradient(135deg,#7C3AED,#4F46E5)":"rgba(255,255,255,0.08)", color: canProceed?"#fff":"rgba(255,255,255,0.3)", border:"none", borderRadius:14, padding:14, fontSize:15, fontWeight:700, cursor: canProceed?"pointer":"not-allowed", fontFamily:"'Nunito',sans-serif", transition:"all 0.2s" }}>
...
function InviteModal({ boardId, boardName, onClose }) {
  ...
  const shareLink = () => {
    if (navigator.share) {
      navigator.share({ title:`Join ${boardName}`, text:"Join my accountability board!", url:inviteLink });
    } else copyLink();
  };
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:300, backdropFilter:"blur(8px)", padding:16, animation:"fadeIn 0.2s" }}>
      <div className="glass" style={{ borderRadius:24, padding:28, width:"100%", maxWidth:400, animation:"slideUp 0.3s ease" }}>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:16 }}>
          <button className="copy-btn" onClick={copyLink} style={{ flex:1 }}>
            {copied ? "✓ Copied!" : "📋 Copy Link"}
          </button>
          <button onClick={shareLink}
            style={{ flex:1, background:"linear-gradient(135deg,#7C3AED,#4F46E5)", color:"#fff", border:"none", borderRadius:10, padding:"10px 16px", cursor:"pointer", fontFamily:"'Nunito',sans-serif", fontSize:13, fontWeight:700 }}>
            🚀 Share
          </button>
        </div>
        ...
        <div style={{ background:"rgba(134,239,172,0.08)", border:"1px solid rgba(134,239,172,0.2)", borderRadius:12, padding:14 }}>
          ...
          <p style={{ fontSize:12, color:"rgba(255,255,255,0.5)", lineHeight:1.8 }}>
            1. Share this link with your partner<br />
            2. They open it and enter their name<br />
            3. They join your board instantly<br />
            4. Everyone's check-ins sync in real-time 🔥
          </p>
        </div>
      </div>
    </div>
      ...
      try {
        ...
        const streakInfo = members.map(m => {
          ...
          while (true) {
            ...
            if ((checkins[`${yyyy}-${mm}-${dd}`]||[]).includes(m.id)) { s++; d.setDate(d.getDate()-1); } else break;
          }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, backdropFilter:"blur(6px)", padding:16, animation:"fadeIn 0.2s" }}>
      <div className="glass" style={{ borderRadius:22, padding:26, width:"100%", maxWidth:380, animation:"slideUp 0.3s ease" }}>
        ...
        <select className="m-input" value={category} onChange={e => setCategory(e.target.value)} style={{ marginBottom:14 }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={() => { if(!title.trim()) return; onAdd({ id:Date.now().toString(), title:title.trim(), category, deadline, assignedTo, completed:false, createdAt:getToday() }); onClose(); }}
            style={{ flex:2, background:"linear-gradient(135deg,#7C3AED,#4F46E5)", color:"#fff", border:"none", borderRadius:10, padding:"12px", cursor:"pointer", fontFamily:"'Nunito',sans-serif", fontSize:14, fontWeight:700 }}>
            Add Goal
          </button>
        </div>
      </div>
...
export default function AccountabilityBoard() {
  const [boardId, setBoardId] = useState(() => {
    ...
    return urlParams.get("board") || localStorage.getItem("acc_board_id") || null;
  });
  ...
  // Load board + real-time subscription
  useEffect(() => {
  ...
  const getStreak = useCallback((memberId) => {
    ...
    while (true) {
      const yyyy = d.getFullYear(); const mm = String(d.getMonth()+1).padStart(2,"0"); const dd = String(d.getDate()).padStart(2,"0");
      if ((checkins[`${yyyy}-${mm}-${dd}`]||[]).includes(memberId)) { s++; d.setDate(d.getDate()-1); } else break;
    }
    return s;
  }, [board]);
  ...
  const updateBoard = async (updates) => {
    ...
    if (error) { showToast("Sync failed. Check connection.", "error"); console.error(error); }
  };
  ...
  if (!boardId) {
    return <SetupScreen onComplete={(id, mid) => { setBoardId(id); setMyMemberId(mid); setLoading(true); }} />;
  }
  ...
  const toggleCheckin = async (memberId, memberName) => {
    ...
    const newList = already ? dayList.filter(id => id !== memberId) : [...dayList, memberId];
    await updateBoard({ checkins: { ...checkins, [today]: newList } });
    if (!already) showToast(`✅ ${memberName} checked in!`, "success");
  };

  const toggleGoal = async (goalId) => {
    await updateBoard({ goals: goals.map(g => g.id===goalId ? { ...g, completed:!g.completed } : g) });
  };

  const addGoal = async (goal) => {
    await updateBoard({ goals: [...goals, goal] });
    showToast("Goal added! 🎯");
  };

  const deleteGoal = async (goalId) => {
    await updateBoard({ goals: goals.filter(g => g.id !== goalId) });
  };
  ...
  const exportCSV = () => {
    ...
    a.download = `accountability-${today}.csv`; a.click();
    showToast("📥 CSV exported!", "info");
  };

  const resetBoard = () => {
    localStorage.removeItem("acc_board_id");
    localStorage.removeItem("acc_member_id");
    setBoardId(null); setMyMemberId(null); setBoard(null);
  };
      ...
      <div style={{ maxWidth:620, margin:"0 auto", padding:"0 16px 100px" }}>
        ...
        <div style={{ padding:"28px 0 18px", animation:"slideUp 0.5s ease", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          ...
          <div style={{ display:"flex", gap:4, flexShrink:0 }}>
            <button className="icon-btn" style={{ fontSize:20 }} onClick={() => setShowInvite(true)} title="Invite">🔗</button>
        ...
        <div className="glass-card" style={{ padding:"16px 20px", marginBottom:20, animation:"slideUp 0.5s 0.1s ease both", background: allCheckedIn?"rgba(134,239,172,0.1)":"rgba(255,255,255,0.05)", borderColor: allCheckedIn?"rgba(134,239,172,0.3)":"rgba(255,255,255,0.1)" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              <p style={{ fontFamily:"'Outfit',sans-serif", fontSize:17, fontWeight:800, color: allCheckedIn?"#86EFAC":"#F0EAFF" }}>
                {allCheckedIn ? "🎉 Everyone checked in!" : `${todayCheckedIn.length}/${members.length} checked in today`}
              </p>
              <p style={{ fontSize:12, color:"rgba(255,255,255,0.35)", marginTop:2 }}>
                {allCheckedIn ? "Amazing! Keep the streak alive." : "Tap your ring below to check in 👇"}
              </p>
            </div>
            <p style={{ fontFamily:"'Outfit',sans-serif", fontSize:30, fontWeight:900, color: allCheckedIn?"#86EFAC":"rgba(255,255,255,0.15)" }}>
              {members.length > 0 ? Math.round((todayCheckedIn.length/members.length)*100) : 0}%
            </p>
          </div>
        </div>
        ...
        {tab === "board" && (
          <div>
            {members.map((member, i) => {
              ...
              const completedGoals = memberGoals.filter(g => g.completed).length;
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          ...
                          <span style={{ fontSize:12, background: checkedIn?"rgba(134,239,172,0.15)":"rgba(255,255,255,0.07)", color: checkedIn?"#86EFAC":"rgba(255,255,255,0.4)", padding:"2px 10px", borderRadius:20, fontWeight:600 }}>
                            {checkedIn?"✓ Done":"Pending"}
                          </span>
                        </div>
                      </div>
                      <p style={{ fontSize:12, color:"rgba(255,255,255,0.35)", marginTop:3 }}>
                        {memberGoals.length>0 ? `${completedGoals}/${memberGoals.length} goals complete` : "No goals yet"}
                      </p>
                    </div>
                  </div>
                    ...
                    {last7.map(d => {
                      ...
                      return (
                        <div key={d} style={{ flex:1, textAlign:"center" }}>
                          <div style={{ height:32, borderRadius:6, background: done?"rgba(167,139,250,0.2)":"rgba(255,255,255,0.05)", border:`1px solid ${done?"rgba(167,139,250,0.5)":"rgba(255,255,255,0.08)"}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, color: done?"#A78BFA":"rgba(255,255,255,0.1)" }}>
                            {done?"✓":"·"}
                          </div>
                          <p style={{ fontSize:10, color: d===today?"#A78BFA":"rgba(255,255,255,0.25)", marginTop:4, fontWeight: d===today?700:400 }}>
            ...
            {board.mode !== MODES.SOLO && (
              <div className="glass-card" style={{ padding:20, textAlign:"center", borderStyle:"dashed", cursor:"pointer", opacity:0.7 }} onClick={() => setShowInvite(true)}>
                ...
                <p style={{ fontFamily:"'Outfit',sans-serif", fontSize:15, fontWeight:700, color:"#A78BFA" }}>Invite your partner</p>
                      ...
                      return (
                        <div key={goal.id} className="glass-card" style={{ padding:"14px 16px", marginBottom:8, display:"flex", gap:12, alignItems:"center", opacity: goal.completed?0.6:1 }}>
                          <div onClick={() => toggleGoal(goal.id)}
                            style={{ width:26, height:26, borderRadius:"50%", border:`2px solid ${goal.completed?"#86EFAC":"rgba(255,255,255,0.2)"}`, background: goal.completed?"rgba(134,239,172,0.2)":"transparent", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:"#86EFAC", flexShrink:0, transition:"all 0.2s" }}>
                            {goal.completed?"✓":""}
                          </div>
              ...
              {[...members].sort((a,b) => getStreak(b.id)-getStreak(a.id)).map((m, rank) => {
                ...
                return (
                  <div key={m.id} style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 0", borderBottom: rank<members.length-1?"1px solid rgba(255,255,255,0.06)":"none" }}>
                    <span style={{ fontSize:22, width:30, textAlign:"center" }}>{medals[rank]||`#${rank+1}`}</span>
                    <span style={{ fontSize:28 }}>{m.avatar}</span>
                    <div style={{ flex:1 }}>
                      <p style={{ fontFamily:"'Outfit',sans-serif", fontSize:16, fontWeight:800, color:"#F0EAFF" }}>
                        {m.name} {m.id===myMemberId && <span style={{ fontSize:11, color:"#A78BFA" }}>(you)</span>}
                      </p>
              ...
              return (
                <div key={m.id} className="glass-card" style={{ padding:18, marginBottom:12 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12 }}>
                    <p style={{ fontFamily:"'Outfit',sans-serif", fontSize:15, fontWeight:800, color:"#F0EAFF" }}>
                      {m.avatar} {m.name} {m.id===myMemberId && <span style={{ fontSize:11, color:"#A78BFA" }}>(you)</span>}
                    </p>
